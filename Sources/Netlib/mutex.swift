...
//  ACTION OF CONTRACT, NEGLIGENCE OR OTHER TORTUROUS ACTION, ARISING OUT OF OR
...
extension RawMutex {
	/** RECOMMENDATION: until Swift can inline between modules or at least optimize
	 @noescape closures to the stack, if this file is linked into another compilation
	  unit (i.e. linked as part of the CwlUtils.framework but used from another module)
	   it might be a good idea to copy and paste the relevant `fastsync` implementation
	    code into your file (or module and delete `private` if whole module optimization
	     is enabled) and use it instead, allowing the function to be inlined.
~~~
...
/// A basic wrapper around the "NORMAL" and "RECURSIVE" `pthread_mutex_t`
/// (a safe, general purpose FIFO mutex). This type is a "class" type to take
/// advantage of the "deinit" method and prevent accidental copying
